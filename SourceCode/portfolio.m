...
sprintf('Hindsight MACDH: %.3f', investhMACDH)
sprintf('Before optimization: Investment results - MACDH: %.3f, fMACDH: %.3f', investMACDH, investfMACDH)
sprintf('LagBuy: %.3f, LagSell: %.3f Miss: %.3f, Total: %.3f', lagBuy, lagSell, miss, total)

out = nan(101,6);
out(1,:) = [investhMACDH, investfMACDH, lagBuy, lagSell, miss, total];

for i = 2:101
    x = ga(funcFitness, 5, A, B, [], [], lb, ub, [], [1,2,3], gaOpt);
    [macdh, fMACDH, hMACDH] = getfMACDH(inputTest, predOutTest, x(1), x(2), x(3), 1);
    ...
    investfMACDH = investSignal(fMACDHSignal, inputTest, 300000);
    investhMACDH = investSignal(hMACDHSignal, inputTest, 300000);
    out(i, :) = [investhMACDH, investfMACDH, lagBuy, lagSell, miss, total];
end
